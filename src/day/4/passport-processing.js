...
const countValidPassports = (data) => {
    ...
    for (let i = 0; i < passports.length; i++) {
        ...
        const passportkeys = [];
        let isPassportInvalid = false;
        for (let j = 0; j < passportFields.length; j++) {
            const passportKeyValue = passportFields[j].split(':');
            if (passportKeyValue[0] === 'byr') {
                if (passportKeyValue[1] < 1920 || passportKeyValue[1] > 2002) {
                    isPassportInvalid = true;
                    break;
                }
            } else if (passportKeyValue[0] === 'iyr') {
                if (passportKeyValue[1] < 2010 || passportKeyValue[1] > 2020) {
                    isPassportInvalid = true;
                    break;
                }
            } else if (passportKeyValue[0] === 'eyr') {
                if (passportKeyValue[1] < 2020 || passportKeyValue[1] > 2030) {
                    isPassportInvalid = true;
                    break;
                }
            } else if (passportKeyValue[0] === 'hgt') {
                const unit = passportKeyValue[1].substring(passportKeyValue[1].length - 2, passportKeyValue[1].length);
                const regex = RegExp(/^cm|in$/);
                if (!regex.test(unit)) {
                    isPassportInvalid = true;
                    break;
                }
                const val = passportKeyValue[1].substring(0, passportKeyValue[1].length - 2);
                if (unit === 'cm') {
                    if (val < 150 || val > 193) {
                        isPassportInvalid = true;
                        break;
            ...
            } else if (passportKeyValue[0] === 'hcl') {
                const regex = RegExp(/^#[a-f0-9]{6}$/);
                if (!regex.test(passportKeyValue[1])) {
                    isPassportInvalid = true;
                    break;
                }
            } else if (passportKeyValue[0] === 'ecl') {
                const regex = RegExp(/^(amb|blu|brn|gry|grn|hzl|oth)$/);
                if (!regex.test(passportKeyValue[1])) {
                    isPassportInvalid = true;
                    break;
                }
            } else if (passportKeyValue[0] === 'pid') {
            ...
            passportkeys.push(passportKeyValue[0]);
        }
        if (isPassportInvalid) continue;
        if (passportkeys.length === 7 && !passportkeys.includes('cid')) {
            validPassports += 1;
        } else if (passportkeys.length === 8) {
            validPassports += 1;
        }
    }
