...
const countPasswordsValid = (arr) => {
    let validPasswords = 0;
    for (let i = 0; i < arr.length; i++) {
        const instructions = arr[i].split(' ');
        const range = instructions[0].split('-');
        const start = parseInt(range[0]) - 1;
        const end = parseInt(range[1]) - 1;
        const char = instructions[1];
        const str = instructions[2];
        if ((str[start] === char[0] && str[end] !== char[0]) || (str[start] !== char[0] && str[end] === char[0])) {
...
console.log(countPasswordsValid(arr));